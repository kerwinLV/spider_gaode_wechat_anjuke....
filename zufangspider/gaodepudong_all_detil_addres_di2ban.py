...
def save_sql(data_processing_list,addres_id):
    ...
    for i in data_processing_list:
        try:
            ...
            cur = conn.cursor()
            SQL = 'insert into gaodedetil_adddres_third (name,type,address,location,pname,cityname,adname,area_id,tel,children,photos)' \
                  ' value ("%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s")'
            cur.execute(SQL%(i["name"],i["type"],i["address"],i["location"],i["pname"],i["cityname"],i["adname"],addres_id,i["tel"],i["children"],i["photos"]))
            conn.commit()
            print("插入成功")
        except Exception as e:
            isupdataid = 1
